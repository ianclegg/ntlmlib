...
Integration Tests
=================
These tests will only run on Windows. They use the Windows SSPI API to create an NTLM server context which is used by
the tests to validate the NTLM client implementation in python.
You will need pywin32 installed and correctly configured
...
class ClientAuth(_BaseAuth):
    ...
    def __init__(self,
        ...
        if scflags is None:
             # sspicon.ISC_REQ_INTEGRITY|sspicon.ISC_REQ_SEQUENCE_DETECT|\
             scflags = sspicon.ISC_REQ_REPLAY_DETECT | sspicon.ISC_REQ_CONFIDENTIALITY
...
class PythonClientAuth(_BaseAuth):
    """Manages the client side of an SSPI authentication handshake
    """
    def __init__(self, pkg_name):
        self.pkg_info=win32security.QuerySecurityPackageInfo(pkg_name)
        auth = PasswordAuthentication('workgroup', 'developer', 'password', compatibility=4)
...
if __name__=='__main__':
   # Setup the 2 contexts.
    sspi_client = ClientAuth("NTLM")
    sspi_server = ServerAuth("NTLM")
    ...
    while 1:
        ...
        print ":".join("{:02x}".format(ord(c)) for c in sec_buffer[0].Buffer)
        print sec_buffer[0].Buffer
        err, sec_buffer = sspi_server.authorize(sec_buffer)
        if err is not 0:
            print ":".join("{:02x}".format(ord(c)) for c in sec_buffer[0].Buffer)
            print sec_buffer[0].Buffer
        if err == 0:
    ...
    sig = sspi_client.sign(data)
    sspi_server.verify(data, sig)

    data, key = sspi_client.encrypt(data)
    result = sspi_server.decrypt(data, key)
    print "cool!"



# Setup the Windows SSPI Server Context
flags = sspicon.ASC_REQ_EXTENDED_ERROR
